...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    watch: {
      comments: {
        handler(newValue: Comment[], oldValue?: Comment[]): void {
          ...
          this.showLoadComments = newValue.length > 0;
        },
        immediate: true,
        deep: true,
      },
    ...
    methods: {
      ...
      loadComments(): void {
        let maxId = this.allComments.length 
          ? ((this.allComments[this.allComments.length - 1].id as number) - 1) as unknown as string
          : undefined;
